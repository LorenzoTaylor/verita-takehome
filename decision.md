...
Complex queries (upserts, aggregations, locks) meant I needed to see the actual SQL. An ORM hiding that is a liability here. Framework and DB layer were picked together since they have to cooperate under the same async model.
...
These were considered together: the job mechanism and idempotency strategy are coupled. Two distinct problems: event replay (2,000/sec peak) and job double-run (hourly/monthly).
...
Scaling note: the idempotency table is the first thing that breaks at peak load. Migration path: Redis SETNX for dedup, keep the table as an async audit trail.

---

## API key storage

Keys must not be retrievable in plaintext after creation. The question was how ops would identify which key a customer is using without exposing the secret.
...
**Chose: Prefix + hash.** Store a short prefix (e.g. `sk_abc123`) + Argon2 hash of the full key. Show the full key once on creation, never again. Ops can identify which key without seeing the secret.

What breaks: if the prefix is too long it leaks entropy. Keep it to 8 chars max.
...
**Chose: Axum extractor.** Auth resolves the API key to an `AuthenticatedCustomer` type. Every query that touches customer data takes that type and filters by `customer_id` in the SQL. Can't write a handler that skips it without the compiler complaining. No fetch-then-check, just filtered fetch.
...
Usage events are written continuously, so a customer paging through results while ingestion is happening will get duplicate or skipped rows with offset pagination.
...
What breaks: cursors tied to `timestamp` alone break if two events share the same timestamp and straddle a page boundary. Always use `(timestamp, id)` as a composite cursor.
...
**Chose: Price plan table.** Tiers stored as rows with an `effective_from` date. Job reads the plan active at invoice period start, one read per job run, not per event. Future price changes are a data update, not a deploy.
...
**Chose: Flag and hold + credit adjustment.** Late events are written to the DB with a `late` status and excluded from the current window. Ops sees the delta. The adjustment posts as a credit on the next invoice. Closed invoices are never reopened. Customer sees a credit on their next bill with a clear reason.

What breaks: if late events arrive in large batches (e.g. a backfill), the credit can be surprisingly large. Ops anomaly signal should flag credits over a threshold for review before the invoice is finalized.

---

## Frontend structure

Spec allows one or two SPAs. Security isolation matters: ops components must never be reachable from the customer surface.
...
What breaks: shared logic (API client, types) gets duplicated. Fine for this scope. Extract into a shared `packages/common` if it grows.
...
The spec doesn't define how ops users authenticate. Ops console is a separate SPA hitting the same backend.
...
**Chose: Ops user table.** Audit log entries must capture actor. A shared env token makes every ops action look identical in the log. Separate ops accounts give real actor identity. Kept simple: email + hashed password, JWT session, no roles beyond "is ops."
...
**Chose: Delivery ID dedup table.** Processor sends a unique delivery ID in the header. Store it on receipt: `INSERT ... ON CONFLICT DO NOTHING`. Already seen = return 200 and do nothing. Handles replays regardless of invoice state or event type.

What breaks: delivery IDs must be trusted from the header. A forged or missing ID bypasses dedup. Mitigate: signature verification runs before the dedup check, so unsigned requests are rejected first.

---

## Anomaly detection

Spec calls out 10x 30-day average as the example signal. Additional signals in scope: zero usage drop, high-frequency `request_id` (retry loop), invoice spike vs. prior month, API key used from multiple IPs in short window.

**Signals in scope:**
- Usage 10x 30-day average
- Usage drops to zero
...
Folding anomaly detection into the window job would have been simpler. Rejected: mixing aggregation and anomaly logic couples two concerns that should evolve independently (different cadence, different logic).
...
Anomaly job reads from pre-aggregated windows, not raw events, so it's cheap. Each job can be tuned or replaced without touching the others.

What breaks: if the window job fails, the anomaly job runs on stale data and may miss a spike. Fix: anomaly job checks window job last-run timestamp before executing.
...
The window job scans all of `usage_events` every hour. At 500M events/month that's a full table scan on every run. Fix: partition `usage_events` by month. Each run only touches the current partition; past months are read-only. The job stays a full recompute per partition (preserving correctness for late events) without scanning the entire table.

**2. Event ingest write path**
`POST /v1/events` writes synchronously to Postgres. At 2,000 events/sec this works; at 10x the write path backs up and events start dropping, a direct revenue risk. Fix: queue (Kafka or SQS) between the product and the backend. Product enqueues and moves on; a consumer writes to Postgres with retries, spitting to a DLQ after exhausting them. Replay is safe: `request_id` idempotency handles duplicate delivery.

**3. Single Postgres write node**
All writes funnel into one instance. Fix: read replica for all reads. Simple rule: writes go to the primary, everything else goes to the replica. Then partition `usage_events` by customer_id hash if per-partition write throughput becomes the constraint.
